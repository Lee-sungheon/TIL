...
2. user 레코드 생성
   ...
   ```python
   # orm
   User.objects.create(last_name='sh', first_name='lee', age=27, phone='010-1234-5678', balance=700, country='gumi')
   ```

   ```sql
   -- sql
   INSERT INTO users_user VALUES (101, 'sh', 'lee', 27, 'deagu', '010-1234-5678', 500000);
   ```
...
4. 해당 user 레코드 수정
   ...
   ```
      ...
      ```sql
   -- sql
   UPDATE users_user SET first_name="CheolSu" WHERE id=101;
      ```
...
1. 전체 인원 수 
   ...
   ```python
   # orm
   User.objects.count()	# 리턴 값은 갯수
   len(User.objects.all())	# QuerySet 형태로 리턴(유사 리스트)
   ```
...
3. 잔고는 오름차순, 나이는 내림차순으로 10명?
      ...
      ```python
   # orm
   User.objects.order_by('balance', '-age')[5:15]
```
