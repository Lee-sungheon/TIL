...
import { Button, Image, Input, ErrorMessage } from "../components/common";
...
const Signup = ({ navigation }) => {
  ...
  const [photo, setPhoto] = useState(
    "https://firebasestorage.googleapis.com/v0/b/rn-chat-92b89.appspot.com/o/profile.png?alt=media"
  );
  ...
  useEffect(() => {
    setDisabled(
      !(email && password && passwordConfirm && name && !errorMessage)
    );
  ...
  const _handleSigupBtnPress = async () => {
    try {
      spinner.start();
    ...
    <KeyboardAwareScrollView extraScrollHeight={20}>
      <Container>
        ...
        <Input
          ...
          isPassword={true}
          onSubmitEditing={_handleSigupBtnPress}
          onBlur={() => setPasswordConfirm(removeWhitespace(passwordConfirm))}
        />
        <ErrorMessage message={errorMessage} />
        <Button
          title="Sign up"
          onPress={_handleSigupBtnPress}
          disabled={disabled}
        />
      </Container>
